<?php session_start();
	...
	/*  
		...
		This page processes and uploads new photos. 
		
		Please note nearly 100% of code used for uploading photos is found here
...
$destination_folder     = 'UserPhotos/'; //upload directory ends with / (slash)
...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
     ...
     if($image_res){
         ...
         if(normal_resize_image($image_res, $image_save_folder, $image_type, $max_image_size, $image_width, $image_height, $jpeg_quality))
         {
             ...
             echo '<img src="UserPhotos/'.$thumb_prefix . $new_file_name.'" alt="Thumbnail">';
             echo '<br />';
             echo '<img src="UserPhotos/'. $new_file_name.'" alt="Resized Image">';
             echo '</div>';
         }
		 
		 $new_image_name = (string)$image_save_folder;
		 photo_db_add($new_image_name);
...
function photo_db_add($new_file_name)
{
	...
	if(!($stmt1 = $mysqli->prepare("INSERT INTO UserPhotos (filename, set_num, photo_owner) VALUES (?,?,?)"))){
		echo "Prepare failed: ";//  . $stmt1->errno . " " . $stmt1->error;
	}
	if(!($stmt1->bind_param('sii', $new_file_name, $set_num, $owner))){
	...
	if(!$stmt1->execute()){
		...
		echo "Added " . $stmt1->affected_rows . " photo to User Photos.";
	}
