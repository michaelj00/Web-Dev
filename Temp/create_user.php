<?php session_start();
	...
	}else if (strlen($hash) >= 20) 
	{
		...
		if (!($result = $mysqli->prepare("SELECT MealSafeUsers.userName FROM MealSafeUsers WHERE MealSafeUsers.userName = ?")))
		{
		...
		{			//insert into the table. 
			if (!($stmt = $mysqli->prepare("insert into MealSafeUsers (userName, passWord) VALUES ( ?,?)")))
			{
			...
			{			//this else sets the session variables for the new user. 
				...
				$resultGetPkey = $mysqli->prepare("SELECT * FROM MealSafeUsers WHERE userName = ?");
				...
				$pKey = NULL;
				$Mail = NULL;
				if (!$resultGetPkey->bind_result($pKey, $uName, $pWord))
				{
